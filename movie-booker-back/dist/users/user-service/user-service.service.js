...
let UserService = class UserService {
    ...
    async register(registerDto) {
        ...
        if (userExists) {
            throw new common_2.ConflictException("Cet utilisateur existe déja");
        }
    ...
    async login(loginDto) {
        ...
        if (!user) {
            throw new common_3.UnauthorizedException('Email ou mot de passe non valide');
        }
        const PassValide = await bcrypt.compare(password, user.password);
        if (!PassValide) {
            throw new common_3.UnauthorizedException('Email ou mot de non valide.');
        }
